...
class coco(IMDB):
    ...
    def image_path_from_index(self, index):
        ...
        if 'DOTA' in self.data_path:
            # fyk: get 'file_name' from im_ann directly
    ...
    def _print_detection_metrics(self, coco_eval):
        ...
        aps_auc = '\x1b[0;32;40m{} \x1b[0m'.format('%-10.2f'%(100 * ap_default))
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            # minus 1 because of __background__
            ...
            ap = np.mean(precision[precision > -1])
            clses += '{} '.format('%-10s'%cls)
            aps_auc += '{} '.format('%-10.2f'%(100 * ap))
        print clses
        print aps_auc
        ################### fyk end ####################
