"""
code from TextBoxes++: https://github.com/MhLiao/TextBoxes_plusplus/blob/master/examples/text/nms.py
Note: this realization is TOO SLOW, don't use this to train.
"""
...
def polygon_overlaps_py(boxes, query_boxes):
    """
    determine overlaps between boxes and query_boxes
    Intersection over union between shapely polygons.
    :param boxes: n * 8 bounding boxes
    :param query_boxes: k * 8 bounding boxes
    ...
    for k in range(k_):
        for n in range(n_):
            overlaps[n, k] = _polygon_iou(boxes[n, :], query_boxes[k, :])
    return overlaps


def _polygon_iou(list1, list2):
    """
