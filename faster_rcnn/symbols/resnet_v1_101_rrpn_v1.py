# --------------------------------------------------------
# RRPN v1: r-RPN + r-RCNN without RRoI Pooling
...
class resnet_v1_101_rrpn_v1(Symbol):
    def __init__(self):
        """
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight, bbox_rois_for_pool = mx.sym.Custom(rois=rois, gt_boxes=gt_boxes_reshape,
                                                                  op_type='proposal_target_rotate',
                                                                  num_classes=num_reg_classes,
                                                                  batch_images=cfg.TRAIN.BATCH_IMAGES,
                                                                  batch_rois=cfg.TRAIN.BATCH_ROIS,
                                                                  cfg=cPickle.dumps(cfg),
                                                                  output_horizon_rois=True,
        ...
        # for RRPN v1, we only pooling from bbox of quadrangle rois instead of inclined roi pooling
        roi_pool = mx.symbol.ROIPooling(
            name='roi_pool', data=conv_new_1_relu, rois=bbox_rois_for_pool, pooled_size=(7, 7), spatial_scale=0.0625)
