...
class ProposalTargetRotateOperator(mx.operator.CustomOp):
    def __init__(self, num_classes, batch_images, batch_rois, cfg, fg_fraction, output_horizon_rois, output_horizon_target):
        super(ProposalTargetRotateOperator, self).__init__()
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        blobs = sample_rois_rotate(all_rois, fg_rois_per_image, rois_per_image, self._num_classes, self._cfg, gt_boxes=gt_boxes,
                               output_horizon_rois=self._output_horizon_rois, output_horizon_target=self._output_horizon_target,
        ...
        for ind, val in enumerate(blobs):
...
class ProposalTargetRotateProp(mx.operator.CustomOpProp):
    def __init__(self, num_classes, batch_images, batch_rois, cfg, fg_fraction='0.25', output_horizon_rois='False',
                 output_horizon_target='False'):
        super(ProposalTargetRotateProp, self).__init__(need_top_grad=False)
    ...
    def list_outputs(self):
        ...
        if self._output_horizon_rois:
            ...
            if self._output_horizon_target:
                out_blob.extend(['horizon_bbox_targets', 'horizon_bbox_weights'])
    ...
    def infer_shape(self, in_shape):
        ...
        if self._output_horizon_rois:
            h_box_dim = 4
            output_horizon_shape = (rois, h_box_dim + 1)
            out_shape.append(output_horizon_shape)
            if self._output_horizon_target:
                h_bbox_target_shape = (rois, self._num_classes * h_box_dim)
                h_bbox_weight_shape = (rois, self._num_classes * h_box_dim)
                out_shape.extend([h_bbox_target_shape, h_bbox_weight_shape])

        return [rpn_rois_shape, gt_boxes_shape], \
               out_shape

    def create_operator(self, ctx, shapes, dtypes):
        return ProposalTargetRotateOperator(self._num_classes, self._batch_images, self._batch_rois, self._cfg, self._fg_fraction,
                                                self._output_horizon_rois, self._output_horizon_target)
